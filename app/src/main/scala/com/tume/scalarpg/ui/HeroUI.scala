...
class HeroUI extends UIView {
  ...
  override def build: Seq[UIBuilder] = {
    ...
    view += { lab=UIBuilder.label.img(R.drawable.ic_frost).text("4.9%").height(lh).width(rw).alignLeft(lab).below(lab); lab }
    view += { lab=UIBuilder.label.img(R.drawable.ic_lightning).text("8.0%").height(lh).width(rw).alignLeft(lab).below(lab); lab }
    // Res row 2
    view += { lab=UIBuilder.label.img(R.drawable.ic_holy).text("7.0%").height(lh).width(rw).rightOf(first, 0.02f).alignTop(first); first=lab; lab }
    view += { lab=UIBuilder.label.img(R.drawable.ic_arcane).text("7.6%").height(lh).width(rw).alignLeft(lab).below(lab); lab }
    view += { lab=UIBuilder.label.img(R.drawable.ic_poison).text("1.8%").height(lh).width(rw).alignLeft(lab).below(lab); lab }
    view += { lab=UIBuilder.label.img(R.drawable.ic_dark).text("3.1%").height(lh).width(rw).alignLeft(lab).below(lab); lab }
    // Equipment
